...
///
/// {@tool dartpad}
/// This example shows a [ClodSegmentedControl] with an enum type.
///
...
/// See also:
///
///  * [ClodSegmentedControl], a segmented control widget in the style used
///    up until iOS 13.
///  * <https://developer.apple.com/design/human-interface-guidelines/ios/controls/segmented-controls/>
class ClodSegmentedControl<T extends Object> extends StatefulWidget {
  ...
  @override
  State<ClodSegmentedControl<T>> createState() => _SegmentedControlState<T>();
}

class _SegmentedControlState<T extends Object> extends State<ClodSegmentedControl<T>>
    with TickerProviderStateMixin<ClodSegmentedControl<T>> {
  T? _pressedKey;
  ...
  @override
  void didUpdateWidget(ClodSegmentedControl<T> oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    for (final T currentKey in widget.children.keys) {
      ...
      child = MouseRegion(
        // Si es Web y no es la primera columna cambio el cursor a dedito
        cursor: kIsWeb && index > 0 ? SystemMouseCursors.click : MouseCursor.defer,
        child: GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTapDown: index > 0 ? (TapDownDetails event) {
            _onTapDown(currentKey);
          } : null,
          onTapCancel:  index > 0 ? _onTapCancel : null,
          onTap:  index > 0 ? () {
            _onTap(currentKey);
          } : null,
          child: IconTheme(
